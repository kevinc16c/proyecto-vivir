...
class PalabrasClaves extends React.Component {
	...
	showDeleteConfirm = async (id) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.palabras.delete(id);
	...
	showEstadoBaja = async (record) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.palabras.baja(record);
				_this.fetch({
					limite: 10,
					pagina: 1,
				});
	...
	showDelete = async (record) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.palabras.baja({ codigoempl: record.codigoempl });
				_this.fetch({
					limite: 10,
					pagina: 1,
				});
	...
	showHabilitar = async (record) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.palabras.habilitar(record);
				_this.fetch({
					limite: 10,
					pagina: 1,
				});
	...
	fetch = async (params = {}) => {
		try {
			const response = await api.palabras.getAll(queryString.stringify({
				...params,
				query: this.state.q
			}))
			...
			if (response.status === "success") {
				...
				this.setState({
					data: response.data.palabras_clave,
					pagination
				});
			} else {
				this.setState({
            ...
            <div className="container-fluid no-breadcrumb">
				<QueueAnim type="bottom" className="ui-animate">
					<Breadcrumb>
						...
						<Breadcrumb.Item>Palabras claves</Breadcrumb.Item>
					</Breadcrumb>
						...
						<div className="box-body">
							<Row >
								<Col style={{ float: 'left' }} sm={{ span: 12 }} xs={{ span: 24 }}>
									<h4 style={{ marginBottom: 15, display: 'inline-block' }}>Palabras claves</h4>
								</Col>
