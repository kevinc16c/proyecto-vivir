...
export const api = {
	subrubros:{
		async create(data) {
            const response = await fetch(config.URL_ADMIN + '/subrubros', {
            method: 'POST',
            headers: {
        ...
        async baja(id){
            const response = await fetch(config.URL_ADMIN + `/subrubros/baja/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'AUTHORIZATION': `Bearer ${sessionStorage.getItem('token')}`
        ...
        async habilitar(id){
            const response = await fetch(config.URL_ADMIN + `/subrubros/habilitar/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'AUTHORIZATION': `Bearer ${sessionStorage.getItem('token')}`
		...
		async getAll(params = ''){
			const response = await fetch(config.URL_ADMIN + '/subrubros?'+params, {
				method: 'GET',
