...
class Vehiculos extends React.Component {
	...
	showDeleteConfirm = async (id) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.farmacias.delete(id);
	...
	showEstadoBaja = async (record) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.farmacias.baja(record);
				_this.fetch({
					limite: 10,
					pagina: 1,
				});
	...
	showDelete = async (record) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.farmacias.baja({ codigoempl: record.codigoempl });
				_this.fetch({
					limite: 10,
					pagina: 1,
				});
	...
	showHabilitar = async (record) => {
		...
		Modal.confirm({
			...
			cancelText: 'Cancelar',
			async onOk() {
				const response = await api.farmacias.habilitar(record);
				_this.fetch({
					limite: 10,
					pagina: 1,
				});
	...
	fetch = async (params = {}) => {
		try {
			const response = await api.farmacias.getAll(queryString.stringify({
				...params,
				query: this.state.q
			}))
			...
			<div className="container-fluid no-breadcrumb">
				<QueueAnim type="bottom" className="ui-animate">
					<Breadcrumb>
						...
						<Breadcrumb.Item>Farmacias de turno</Breadcrumb.Item>
					</Breadcrumb>
						...
						<div className="box-body">
							<Row >
								<Col style={{ float: 'left' }} sm={{ span: 12 }} xs={{ span: 24 }}>
									<h4 style={{ marginBottom: 15, display: 'inline-block' }}>Farmacias de turno</h4>
								</Col>
